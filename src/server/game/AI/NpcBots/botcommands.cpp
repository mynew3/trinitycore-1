...
public:
    ...
    static bool HandleMainTankCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (!group)
        {
            handler->PSendSysMessage("你必须在队伍中才能使用这个命令　");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        {
            handler->PSendSysMessage("你没有权限使用这个命令　");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        if (!*args)
        {
                    ...
                    if (Unit* u = ObjectAccessor::FindUnit(selection))
                    {
                        ...
                        for (Group::MemberSlotList::const_iterator itr = members.begin(); itr != members.end(); ++itr)
                        {
                            ...
                            if (Player* player = ObjectAccessor::FindPlayer(itr->guid))
                            {
                                ...
                                chp.PSendSysMessage("MT位置为 %s.", u->GetName().c_str());
            ...
            if (Unit* unit = bot_ai::GetBotGroupMainTank(group))
            {
                ...
                handler->PSendSysMessage("MT是 %s (%s%s).", unit->GetName().c_str(), (bot ? "npcbot" : "player"), (unit->IsAlive() ? "" : ", dead"));
                handler->SetSentErrorMessage(true);
                return true;
            }
    ...
    static bool HandleNpcBotDistanceCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (!*args)
        {
            if (owner->HaveBot())
            {
                handler->PSendSysMessage("机器人距离设置为 %u　", owner->GetBotFollowDist());
                handler->SetSentErrorMessage(true);
                return false;
            }
            handler->PSendSysMessage(".npcbot juli");
            handler->PSendSysMessage("设置机器人与你的最远距离.　");
        ...
        if (dist >= 0 && dist <= 75)
        {
            ...
            handler->PSendSysMessage("机器人的跟随距离设置为 %u　", dist);
            return true;
        }
        handler->SendSysMessage("跟随距离必须为0 - 75 之间.　");
        handler->SetSentErrorMessage(true);
        return false;
    }
    ...
    static bool HandleNpcBotRemoveCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        if (!guid)
        {
            ...
            handler->PSendSysMessage("移除你的机器人.　");
            handler->SetSentErrorMessage(true);
            return false;
        }
        if (guid == owner->GetGUID())
        {
            if (owner->HaveBot())
            {
                ...
                if (!owner->HaveBot())
                {
                    handler->PSendSysMessage("机器人已被删除.　");
                    handler->SetSentErrorMessage(true);
                    return true;
                }
                handler->PSendSysMessage("未删除一些机器人!　");
                handler->SetSentErrorMessage(true);
                return false;
            }
        ...
        if (cre && cre->GetIAmABot())
        {
            ...
            {
                handler->PSendSysMessage("你只能删除你的机器人　");
                handler->SetSentErrorMessage(true);
                return false;
            }
        ...
        handler->PSendSysMessage("你必须选择你自己的机器人!　");
        handler->SetSentErrorMessage(true);
        return false;
    }
    ...
    static bool HandleNpcBotResetCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        if (master && master->GetGUID() == owner->GetGUID())
        {
            ...
            {
                handler->PSendSysMessage("战斗中无法刷新机器人!　");
                handler->SetSentErrorMessage(true);
                return false;
            }
            if (!master->HaveBot())
        ...
        handler->PSendSysMessage(".npcbot reset");
        handler->PSendSysMessage("刷新你的机器人,必须机器人卡战斗了,不能在战斗中使用!　");
        handler->SetSentErrorMessage(true);
        return false;
    }
    ...
    static bool HandleNpcBotReviveCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        {
            handler->PSendSysMessage("复活命令被关闭　");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        {
            handler->PSendSysMessage("机器人复活不能在PVP中使用　");
            handler->SetSentErrorMessage(true);
            return false;
        }
        if (owner->IsInFlight())
        {
            handler->PSendSysMessage("飞行中不能使用机器人复活　");
            handler->SetSentErrorMessage(true);
            return false;
        }
        if (owner->HaveBot())
        {
            ...
            handler->PSendSysMessage("机器人复活　");
            handler->SetSentErrorMessage(true);
            return true;
        }
        handler->PSendSysMessage(".npcbot fuhuo");
        handler->PSendSysMessage("让你的机器人复活　");
        handler->SetSentErrorMessage(true);
        return false;
    }
    ...
    static bool HandleNpcBotAddCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (!*args || sel != owner->GetGUID())
        {
            ...
            handler->PSendSysMessage("添加一个机器人,目标必须选择你自己　");
            handler->SetSentErrorMessage(true);
            return false;
        }
        if (owner->RestrictBots())
        {
            handler->GetSession()->SendNotification("这个地方不能使用机器人　");
            handler->SetSentErrorMessage(true);
            return false;
        }
        if (owner->GetGroup())
        ...
        {
            handler->PSendSysMessage("机器人超过上限了　");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        {
            handler->PSendSysMessage("想要召唤死亡骑士必须等级达到55　");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        {
            if (owner->IsInCombat())
                handler->PSendSysMessage("机器人已经创建 (%s). 将在你脱离战斗时候出现　", owner->GetName().c_str());
            else
                handler->PSendSysMessage("机器人已被创建 (%s)　", owner->GetName().c_str());
            handler->SetSentErrorMessage(true);
            return true;
        }
        handler->PSendSysMessage("未知原因创建失败　");
        handler->SetSentErrorMessage(true);
        return false;
    }
};
