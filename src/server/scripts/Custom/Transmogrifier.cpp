...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (sT->GetEnableTransmogInfo())
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, session->GetTrinityString(11144), EQUIPMENT_SLOT_END+9, 0);
...
#ifdef PRESETS
        if (sT->GetEnableSets())
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, session->GetTrinityString(11145), EQUIPMENT_SLOT_END+4, 0);
#endif
        player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, session->GetTrinityString(11146), EQUIPMENT_SLOT_END+2, 0, session->GetTrinityString(11147), 0, false);
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, session->GetTrinityString(11148), EQUIPMENT_SLOT_END+1, 0);
        ...
        case EQUIPMENT_SLOT_END+4: // Presets menu
            {
                ...
                if (sT->GetEnableSetInfo())
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, session->GetTrinityString(11149), EQUIPMENT_SLOT_END+10, 0);
                ...
                if (sT->presetByName[player->GetGUID()].size() < sT->GetMaxSets())
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, session->GetTrinityString(11150), EQUIPMENT_SLOT_END+8, 0);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, session->GetTrinityString(11151), EQUIPMENT_SLOT_END+1, 0);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
        ...
        case EQUIPMENT_SLOT_END+6: // view preset
            {
                ...
                player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, session->GetTrinityString(11152), EQUIPMENT_SLOT_END+5, action, session->GetTrinityString(11153)+sT->presetByName[player->GetGUID()][action], 0, false);
                player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, session->GetTrinityString(11154), EQUIPMENT_SLOT_END+7, action, session->GetTrinityString(11155)+sT->presetByName[player->GetGUID()][action]+"?", 0, false);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, session->GetTrinityString(11151), EQUIPMENT_SLOT_END+4, 0);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
        ...
        case EQUIPMENT_SLOT_END+8: // Save preset
            {
                ...
                if (canSave)
                    player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, session->GetTrinityString(11156), 0, 0, session->GetTrinityString(11157), cost*sT->GetSetCostModifier()+sT->GetSetCopperCost(), true);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, session->GetTrinityString(11158), sender, action);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, session->GetTrinityString(11151), EQUIPMENT_SLOT_END+4, 0);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
        ...
        if (oldItem)
        {
            ...
            for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; ++i)
            {
                ...
                if (sT->GetFakeEntry(oldItem->GetGUID()) == newItem->GetEntry())
                    continue;
                ++limit;
                player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, sT->GetItemIcon(newItem->GetEntry(), 30, 30, -18, 0)+sT->GetItemLink(newItem, session), slot, newItem->GetGUIDLow(), session->GetTrinityString(11159)+sT->GetItemIcon(newItem->GetEntry(), 40, 40, -15, -10)+sT->GetItemLink(newItem, session)+ss.str(), price, false);
            }
            ...
            for (uint8 i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END; ++i)
            {
                ...
                for (uint32 j = 0; j < bag->GetBagSize(); ++j)
                {
                    ...
                    if (sT->GetFakeEntry(oldItem->GetGUID()) == newItem->GetEntry())
                        continue;
                    ++limit;
                    player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, sT->GetItemIcon(newItem->GetEntry(), 30, 30, -18, 0)+sT->GetItemLink(newItem, session), slot, newItem->GetGUIDLow(), session->GetTrinityString(11160)+sT->GetItemIcon(newItem->GetEntry(), 40, 40, -15, -10)+sT->GetItemLink(newItem, session)+ss.str(), price, false);
                }
            }
        }

        player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, session->GetTrinityString(11161), EQUIPMENT_SLOT_END+3, slot, session->GetTrinityString(11162), 0, false);
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, session->GetTrinityString(11158), EQUIPMENT_SLOT_END, slot);
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, session->GetTrinityString(11151), EQUIPMENT_SLOT_END+1, 0);
        player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
    }
};
    ...
    void OnLogin(Player* player)
    {
        ...
        if (result)
        {
            do
            {
                ...
                {
                    sLog->outError("sql.sql", "Item entry (Entry: %u, itemGUID: %u, playerGUID: %u) does not exist, ignoring.", fakeEntry, GUID_LOPART(itemGUID), player->GetGUIDLow());
...
public:
    ...
    {
        sT->LoadConfig(false);
        sLog->outInfo("server.loading", "Deleting non-existing transmogrification entries...");
