...
INSERT INTO `command` (name, permission, help) VALUES ('account email', 0, 'Syntax: .account email $oldemail $currentpassword $newemail $newemailconfirmation\r\n\r\n Change your account email. You may need to check the actual security mode to see if email input is necessary for password change');

DELETE FROM `command` WHERE name = 'account set sec email';
INSERT INTO `command` (name, permission, help) VALUES ('account set sec email', 3, 'Syntax: .account set sec email $accountname $email $emailconfirmation\r\n\r\nSet the email for entered player account.');

DELETE FROM `command` WHERE name = 'account set sec regmail';
INSERT INTO `command` (name, permission, help) VALUES ('account set sec regmail', 4, 'Syntax: .account set sec regmail $account $regmail $regmailconfirmation\r\n\r\nSets the regmail for entered player account.');
